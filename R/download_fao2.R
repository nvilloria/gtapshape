...
#'
#' @export
download_fao2 <- function(gtap_year, workdir_dir=getwd()) {

    fao_production <- FAOSTAT::get_faostat_bulk(code = "QCL", data_folder =   tempdir())
    producer_prices <- FAOSTAT::get_faostat_bulk(code = "PP", data_folder = tempdir())
    fao_val_production <- FAOSTAT::get_faostat_bulk(code = "QV", data_folder = tempdir() )
    land_cover <- FAOSTAT::get_faostat_bulk(code = "RL", data_folder = tempdir())

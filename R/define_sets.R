...
#'
#' @param reg_set_file Data frame containing the set of regions
...
#'
#' @export
define_sets <- function(reg_set_file, subnat_bound_file, tmp_dir=getwd()) {
  ...
  saveRDS(set_REG_list, file = file.path(tmp_dir, 'tmp/sets/set_REG.rds'))
  ...
  filename <- paste0(tmp_dir, '/tmp/sets/reg_set.txt')
  ...
  saveRDS(subnat_bound_key, file = file.path(tmp_dir, 'tmp/subnat_name_key.rds'))
  ...
  #Save
  saveRDS(set_BIO_list, file = file.path(tmp_dir, 'tmp/sets/set_BIO.rds'))
  #Output the set as a GEMPACK txt file
  hmd <- c(paste0(length(set_BIO_list), ' Strings Length 10 Header "BIO" LongName "Set BIO subnational areas" ;'))
  filename <- paste0(tmp_dir, '/tmp/sets/biome_set.txt')
  write.table(hmd, file=filename, row.names=FALSE, col.names=FALSE, quote = FALSE)
  ...
  filename <- paste0(tmp_dir, '/tmp/sets/biome_endw_set.txt')
  write.table(hmd, file=filename, row.names=FALSE, col.names=FALSE, quote = FALSE)
  ...
  filename <- paste0(tmp_dir, '/tmp/sets/crp9_set.txt')
  write.table(hmd, file=filename, row.names=FALSE, col.names=FALSE, quote = FALSE)
  ...
  filename <- paste0(tmp_dir, '/tmp/sets/lcov_set.txt')
  write.table(hmd, file=filename, row.names=FALSE, col.names=FALSE, quote = FALSE)

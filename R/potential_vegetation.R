...
#'
#' This function creates rasters for potential vegetation.
#'
#' @param pot_veg_file Path to the raster file indicating potential vegetation classes
#' @param tmp_dir Location of the tmp dir create using gtap_setup function. The default is the current working directory set by getwd()
...
#' @export
potential_vegetation <- function(pot_veg_file, tmp_dir=getwd()) {
...
#Now we need to reclassify them into four categories as in Baldoz (2009)
  ...
  #Save
  terra::writeRaster(pot_veg_4cat, filename = file.path(tmp_dir, "tmp/rasters/potential_veg.tif"), overwrite = TRUE)
...
#Create four rasters, one for each category
  ...
  terra::writeRaster(forest_pot_veg, filename = file.path(tmp_dir, "tmp/rasters/forest_pot_veg.tif"), overwrite = TRUE)
  ...
  terra::writeRaster(shrubland_pot_veg, filename = file.path(tmp_dir, "tmp/rasters/shrubland_pot_veg.tif"), overwrite = TRUE)
  ...
  terra::writeRaster(savanna_grass_pot_veg, filename = file.path(tmp_dir, "tmp/rasters/savanna_grass_pot_veg.tif"), overwrite = TRUE)
  ...
  terra::writeRaster(other_pot_veg,  filename = file.path(tmp_dir, "tmp/rasters/other_pot_veg.tif"), overwrite = TRUE)

...
author: "Micah Cameron-Harp & Nelson B. Vileloria"
date: "`r Sys.Date()`"
...
##
.i <- timber.rents.by.iso.2000

f <- shares[['cover']] %>% filter(gsc3=="Forest")

b <- share.out.sectors.to.subnatbound(
            iso.data = .i,
            shares = f,
            units = unique(.i$unit),
...
```{r, eval = FALSE}
write.gtaplulc.har(gsc3.by.iso, file="test1.har")
```

...
Second, we reproject the raster so it has the same Coordinate Reference System (EPSG 4326) and resolution (5 arc-minutes) as the other data. To do this, we use the \link[gtapshape]{make_global_raster} and \link[gtapshape]{apply_globa_raster_properties} functions.to make an empty raster with the target CRS and resolution. 
```{r, eval = FALSE}
  #Create a new global raster using the "make_global_raster" function with the
  ...
  #raster with the same resolution and crs (projection).
    global_rast <- make_global_raster()
...
First, we load in the cropland and pastureland rasters. We then use the \link[gtapshape]{make_global_raster} and \link[gtapshape]{apply_global_raster_properties} to ensure the resulting data have the target CRS (EPSG 4326) and resolution. 
```{r, eval = FALSE}

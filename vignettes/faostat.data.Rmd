...
We combine the producer prices with the data on crop production to generate value of production data. When a country is missing a price for a given crop, we use the world price instead. The world price is created by taking an average of all non-missing prices for a given year. The value of output variable is created by multiplying the price in USD/ton by the quantity of output in tons, and then dividing by 1000. The resulting values indicate the value of production in thousands of USD. Last, we use the \link[usethis]{use_data} function to save the dataset in the *gtapshape* package and create an accompanying description file. 
...
The livestock data from FAOSTAT require cleaning as well. First, we need to drop data that represents aggregate quantities across species. Then, we create a mapping from the species in the FAO data to the GTAP livestock categories and aggregate the FAOSTAT data to these categories. Last, we use the \link[usethis]{use_data} function to save the dataset in the *gtapshape* package and create an accompanying description file. 
...
Now we join the crop and livestock value of production data so the resulting dataframe has data for all three GTAP categories (ctl, rmk, and wol). Last, we use the \link[usethis]{use_data} function to save the dataset in the *gtapshape* package and create an accompanying description file. 

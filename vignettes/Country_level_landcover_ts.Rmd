...
vignette: >
  %\VignetteIndexEntry{Process annual, country-level land cover data}
  %\VignetteEngine{knitr::rmarkdown}
  %\VignetteEncoding{UTF-8}
---

FAOSTAT provides annual data on cropland and pasture areas. The
...
2000 documented HERE is used to estimate the shares of land covers
...
documented [[here]] and available using lazy load gtapshape for easy
load.
...
iso.land.cover.2011.2022 <- left_join(unasigned.land.area.by.year,
    ...
    pivot_longer(
        cols = c("cropland","pasture", "urban", "forest",
                 "other", "grassland", "shrubland"),
        names_to = "use",
        values_to = "value")
```
...
```
Finally, a note over country coverage: The gridded land cover data for 2000 covers 185 countries while FAOSTAT's annual country-level data covers 236 countries. There are 171 countries with data in both datasets, which are the ones for which this update work. The countries for which we cannot assign land cover data are small countries mostly captured by the aggregated GTAP regions `X??`:
